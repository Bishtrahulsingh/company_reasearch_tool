...
def _parse_json(data:str):
    try:
        ...
        matched_data = re.search('```(?:json)?\s*([\s\S]*?)```',data)

        if matched_data:
            try:
                return json.loads(matched_data.group(1).strip())
            except JSONDecodeError:
                raise Exception('invalid json data found')
...
def _extract_tokens(response)->Tuple[int,int]:
    """This is for extracting the input and output tokens"""
...
def _extract_sources(observations:list):
    ...
    {"result": "...", "source": "abc.com", "latency_ms": 10}
...
def build_agent_graph(system_prompt:str, tools:dict , max_steps:int=5):
    ...
    async def act(state:AgentState):
        ...
        """
        There may be 3 cases when tool is not present in list, tool exists and tool timeout
        """
